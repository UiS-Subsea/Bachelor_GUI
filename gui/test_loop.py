...
class Window(QMainWindow):
    def __init__(self, pipe_conn_only_rcv, queue: multiprocessing.Queue, t_watch: Threadwatcher, id:int, parent=None):
        ...
        #Queue and pipe
        self.queue: multiprocessing.Queue = queue # queue is a queue that only receives data
    ...
    def buttonClick(self):
        ...
        #fix communicate
    def receive_sensordata(self,conn): #conn is a pipe connection that only receives data
        self.communicate = Communicate() #Create a new instance of the class Communicate
        self.communicate.data_signal.connect(self.decide_gui_update) #Connect the signal to the function that decides what to do with the sensordata
        while self.t_watch.should_run(self.id):#While the threadwatcher says that the thread should run
    ...
    def gui_manipulator_update(self, sensordata):
        ...
        if sensordata[3]:
            if sensordata[0] != 0: # åpne/lukke manipulator
                self.update_round_percent_visualizer(round(sensordata[0]*0.35), self.label_percentage_mani_1)
            elif sensordata[2] != 0: # rotere manipulator
                self.update_round_percent_visualizer(round(sensordata[2]*0.35), self.label_percentage_mani_2)
            elif sensordata[1] != 0: # inn ut med manipulator1
                self.update_round_percent_visualizer(round(sensordata[1]*0.35), self.label_percentage_mani_3)
