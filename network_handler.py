...
class Network:
    ...
    # def send(self, string_to_send: str) -> None:
    #     if self.conn is None and not self.waiting_for_conn:
    #         raise Exception("Client tried sending with a non existing connection")

    #     while self.waiting_for_conn:
    #         # Prints a message to console that tells the user that we are waiting for a connection
    #         number_of_periods = 0
    #         print("waiting for conn"+(number_of_periods*"."), end="                 \r")
    #         number_of_periods += 1
    #         if number_of_periods > 5:
    #             number_of_periods = 0
    #         time.sleep(0.3)
    #     try:
    #         if type(string_to_send) != str:
    #             print(f"tried sending something that was not a string: {string_to_send = } type: {type(string_to_send)}")
    #             raise TypeError("Wrong type for sending data")
    #         bytes_to_send = self.string_to_bytes(string_to_send)                
    #         self.conn.sendall(bytes_to_send)
    #     except (socket.error, BrokenPipeError)  as e:
    #         print(f"Tried sending, but got an error \n{e}")
    #         print(f"conn = {self.conn}, waiting for conn: {self.waiting_for_conn}")
