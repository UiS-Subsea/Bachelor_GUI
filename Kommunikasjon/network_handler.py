...
class Network:
    ...
    def beat(self):
        while self.running:
            heartbeat_packet = bytes(json.dumps("*"), "utf-8") + bytes(
                json.dumps("heartbeat"), "utf-8") + bytes(json.dumps("*"), "utf-8")
    ...
    def new_conn(self):
        ...
        if self.is_server:
            ...
            wait_for_conn = threading.Thread(
                target=self.wait_for_conn, daemon=True)
            wait_for_conn.start()
        else:
            print(
                f"Client is trying to connect to {self.connect_addr, self.port}")
            while True:
                try:
            ...
            print(
                f"connection from client has been established. conn: {self.conn}")
    ...
    def send(self, bytes_to_send: bytes) -> None:
        if self.conn is None and not self.waiting_for_conn:
            raise Exception(
                "Client tried sending with a non existing connection")
        # print(f"waiting for conn: {self.waiting_for_conn}")
        wait_counter = 0
        while self.waiting_for_conn:
            print("waiting for conn"+(wait_counter*"."),
                  end="                 \r")
            wait_counter += 1
            if wait_counter > 5:
                wait_counter = 0
        ...
        try:
            if type(bytes_to_send) != bytes:
                print(
                    f"tried sending something that was not bytes: {bytes_to_send = } type: {type(bytes_to_send)}")
                exit(1)
...
if __name__ == "__main__":
    ...
    if sys.platform != "win32":
        ...
        client_conn = Network(
            is_server=True, bind_addr="0.0.0.0", connect_addr="10.0.0.3")
        while True:
