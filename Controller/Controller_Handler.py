...
class Controller:
    def __init__(
        self,
        queue_to_rov: multiprocessing.Queue,
        manual_flag,
        ...
        while True:
            try:
                ...
                for sec in range(5, 0, -1):
                    sys.stdout.write(
                        "\r"
                        + f"Shut down and connect controller/s before starting! {sec}"
                    )
                    time.sleep(1)
                    sys.stdout.flush()
        if pygame.joystick.get_count() == 1:
    ...
    def normalize_joysticks(self, event):
        ...
        if event.axis == 1:
            return self.deadzone_adjustment(
                -round(
                    (
                        2
                        * (event.value - -self.controller_stop_point)
                        / (self.controller_stop_point - -self.controller_stop_point)
                        - 1
                    )
                    * 100
                )
    ...
    def get_events_loop(
        self, t_watch: Threadwatcher, id: int, debug=False, debug_all=False
    ):
        """get_events_loop collects all the events and updates the buttons,
        dpad, and joystick values. It then sends it to the queue if it is not local"""
            ...
            for event in pygame.event.get():
                ...
                if event.type == DPAD:  # dpad (both up and down)
                    if event.joy == ROV_CONTROLLER_ID:
                        self.rov_dpad = (
                            event.value
                        )  # BLIR DET BRUKT ELLER ER DET KNAPP?
                ...
                if event.type == BUTTON_DOWN:  # button down
                    if event.joy == ROV_CONTROLLER_ID:
                        ...
                        self.mani_buttons[15] = (
                            -self.mani_buttons[12]
                        ) + self.mani_buttons[11]
                ...
                if event.type == JOYSTICK:  # joystick movement JOYSTICK
                    if event.joy == ROV_CONTROLLER_ID:
                        ...
                        self.rov_joysticks[6] = (
                            self.rov_joysticks[5] - self.rov_joysticks[2]
                        )
                        ...
                        self.mani_joysticks[6] = (
                            self.mani_joysticks[5] - self.mani_joysticks[2]
                        )
...
def run(
    queue_to_rov, manual_flag, t_watch: Threadwatcher, id, debug=True, debug_all=True
):
    # debug_all = True
